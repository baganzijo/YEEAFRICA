      ...
      {menuOpen && (
        <div ref={overlayRef} className="fixed inset-0 bg-black/30 backdrop-blur-sm z-40">
          <div ref={menuRef} className="fixed right-0 top-0 w-3/5 h-full bg-gray-950 dark:bg-gray-950 p-6 z-50 flex flex-col">
            ...
            <nav className="flex flex-col gap-4 text-white dark:text-white">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
              ...
              {!session ? (
                <>
                  <button
                    ...
                    className="w-full text-left px-4 py-2 rounded hover:bg-blue-700 dark:hover:bg-gray-800"
                  >
                    Login
                  </button>
                  <button
